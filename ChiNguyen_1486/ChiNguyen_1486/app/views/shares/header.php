            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                    ...
                    <?php if (SessionHelper::isAdmin()): ?>
                        <li class="nav-item admin-only"><a class="nav-link" href="/Category">Danh mục</a></li>
                    <?php endif; ?>
                </ul>
                    ...
                    <?php if (SessionHelper::isLoggedIn()): ?>
                        ...
                        <div class="dropdown">
                            <button class="btn btn-light btn-sm dropdown-toggle d-flex align-items-center gap-2" type="button" id="userDropdownMenu" data-bs-toggle="dropdown" aria-expanded="false">
                                <?php if (!empty($_SESSION['avatar']) && file_exists(BASE_PATH . '/' . $_SESSION['avatar'])): ?>
                                    <img src="/<?php echo htmlspecialchars($_SESSION['avatar']); ?>" class="rounded-circle border" style="width: 24px; height: 24px; object-fit: cover;">
                                <?php else: ?>
                            ...
                            <ul class="dropdown-menu dropdown-menu-end rounded-3 shadow mt-2" aria-labelledby="userDropdownMenu">
                                <li class="px-3 py-2 border-bottom">
                                    <div class="d-flex align-items-center gap-2">
                                        <?php if (!empty($_SESSION['avatar']) && file_exists(BASE_PATH . '/' . $_SESSION['avatar'])): ?>
                                            <img src="/<?php echo htmlspecialchars($_SESSION['avatar']); ?>" class="rounded-circle border" style="width: 36px; height: 36px; object-fit: cover;">
                                        <?php else: ?>
                                            <i class="bi bi-person-badge-fill text-primary fs-4"></i>
                                        <?php endif; ?>
                                         <div>
                                             <div class="fw-bold"><?php echo $displayName; ?></div>
                                             <div class="text-muted small mb-1" style="font-size: 11px;"><?php echo htmlspecialchars($_SESSION['email'] ?? '', ENT_QUOTES, 'UTF-8'); ?></div>
                                ...
                                <?php endif; ?>
                                <?php if ($userRole === 'admin'): ?>
                                    <li class="admin-only"><a class="dropdown-item d-flex align-items-center gap-2" href="/Admin"><i class="bi bi-speedometer2"></i> Dashboard Quản trị</a></li>
                                    <li class="admin-only"><a class="dropdown-item d-flex align-items-center gap-2" href="/Admin/orders"><i class="bi bi-receipt"></i> Quản lý đơn hàng</a></li>
                                    <li class="admin-only"><a class="dropdown-item d-flex align-items-center gap-2" href="/Account/users"><i class="bi bi-people-fill"></i> Quản lý thành viên</a></li>
                                <?php endif; ?>
                                <li><a class="dropdown-item d-flex align-items-center gap-2 text-danger" id="logout-link" href="/Account/logout"><i class="bi bi-box-arrow-right"></i> Đăng xuất</a></li>
                            </ul>
                        </div>
<script>
document.addEventListener("DOMContentLoaded", function() {
    // Rà soát và ẩn/xóa hoàn toàn các liên kết quản trị trên client side nếu không phải admin
    ...
    if (token) {
        try {
            ...
            if (payload && payload.data && payload.data.role) {
                clientRole = payload.data.role;
            } else if (payload && payload.role) {
                clientRole = payload.role;
            }
        } catch (e) {
            console.error('Lỗi giải mã token JWT:', e);
        }
    }
    
    // Nếu vai trò không phải admin (user hoặc guest), xóa các phần tử admin-only khỏi DOM
    if (clientRole !== 'admin') {
